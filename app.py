...
# Load the encryption key from an environment variable or generate if not available
...
def logout():
    if session.get("logged_in"):
        ...
        session.pop('encrypted_password', None)
...
def get_slanje():
    if session.get("logged_in"):
        # Dobivanje enkriptirane lozinke iz sesije
        encrypted_password = session.get("encrypted_password")
        
        # Dekriptiranje lozinke
        decrypted_password = cipher_suite.decrypt(encrypted_password).decode()
        return send_both_mails(decrypted_password)
