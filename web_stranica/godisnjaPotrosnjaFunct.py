...
def get_consumption():
    ...
    result = cursor.execute(f"SELECT Razdoblje_obracun, Potrosnja_hv FROM Obracun WHERE ID_korisnik={user} AND razdoblje_obracun LIKE '{year}%'").fetchall()
    conn.close()
    # Vrijednosti koje se vracaju za graf 
    consumption = [{'datum': format_date(row[0]), 'potrosnja': row[1]} for row in result]
